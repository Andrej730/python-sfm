...
def groupCreateControlledValue(group, name, value, animset, shot):
	tempControl, tempValue = sfmUtils.CreateControlledValue(
		name, "value", vs.AT_FLOAT, value, animset, shot)
	group.AddControl(tempControl)
#end

def groupGetControlledValueAtTime(group, name, time):
	return GetValueAtTime(
		group.FindControlByName(name, False).channel, time)
#end
...
class RigidbodyRig:
	...
	def readFromGroup(self, group, time):
		...
		print '@143 %s%s' % (name_handle, suffix)
