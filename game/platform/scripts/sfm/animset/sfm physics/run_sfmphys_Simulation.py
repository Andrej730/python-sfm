...
if len(sfmClipEditor.GetSelectedShots()) == 1:
	...
	QtGui.QMessageBox.critical(
		None, 'ERROR - run_sfmphys_simulation.py',
		'You have to select exactly one shot.')
	raise Exception("phys_simulate.py: More than one shot selected.")
...
if (timeSelection.IsEitherInfinite()):
	QtGui.QMessageBox.critical(
		None, 'ERROR - run_sfmphys_simulation.py',
		'You have to select finite time range.')
	raise Exception("phys_simulate.py: Non finite time range selected.")
...
if (len(rigidBodies) == 0 and len(softBodies) == 0):
	QtGui.QMessageBox.critical(
		None, 'ERROR - run_sfmphys_simulation.py',
		'No animation sets with a rig_physics found.')
...
window.setWindowTitle('Physics simulation')
